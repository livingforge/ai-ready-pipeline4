---
name: arp4
description: 既存資産（Excel・Word・PowerPoint・PDF・CSV の設計書とソースコード）を起点に、仕様をリレーショナルな正本データへ変換するパイプライン。機械が資料をパース結果（編集可・git 管理の Markdown）にし、エージェントがそれを読んで整理結果（型付きレコード）を書き、機械が凍結・検証してから正本（items / relations）を組み立て、設計書を生成する。意味の判断は整理層だけが行い、パースと構築は照合・採番・整合性しかやらない。「Excel の設計書を取り込んで」「Word・PowerPoint・PDF の資料を取り込んで」「既存資産から仕様を起こして」「仕様をデータとして管理して」「テーブル定義書・詳細設計書を生成して」「トレーサビリティを取って」で使う。
license: MIT
---
...
**根は `.arp/` を置くリポジトリ** ―― 仕様の持ち主であり、成果物を残したい先である。
**作業場所と同じでも構わない**（対象のリポジトリをそのまま開いているなら、その直下が
根である）。**禁じているのは一致そのものではなく、cwd に決めさせること**である。

```bash
R=/path/to/対象のリポジトリ                  # ★ ここに .arp/ ができる
arp4 init --root "$R"                        # ← 全コマンドに付ける
```

一時フォルダ（セッション用の作業ディレクトリ・`/tmp`）は根にしない。**対象が
どのリポジトリか決められないときは、そこで止めて利用者に聞く** ―― 置き場は
あとから直せない（`.arp/` を作り直すことになる）。
...
**通らなければ [arp4-setup](../arp4-setup/SKILL.md) へ。**ここでは切り分けない ――
`arp4` は console script なので、**入っていても PATH に無い**ことがある（仮想環境の
中にしかない・`python` が別の版を指している）。**ソースリポジトリの中にいても同じで、
ソースがあることと入っていることは別である。**

arp4-setup は起動コマンドを 1 つ決めて返す。**それが `arp4` 以外になったら、この
文書の `arp4` は全部そのコマンドに読み替える**（`--root` は読み替えない）。導入が
要るなら環境を変えることになるので、利用者に断ってから行う。
