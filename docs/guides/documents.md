...
- 行・列の追加削除は、Excel自身の挿入・削除と同じ結果になるようにセル位置を持つ部品を更新します。対象は全シートの数式（共有数式は必要に応じて個別の数式に展開）、名前付き範囲・印刷範囲・印刷タイトル、条件付き書式・入力規則（x14拡張を含む）、ハイパーリンクのセル、オートフィルターと並べ替え、結合セル、列幅、スパークライン、Excelテーブル、ピボットの参照元範囲と配置、グラフ系列、図形のアンカーです。範囲の内側への挿入は範囲を広げ、一部の削除は範囲を縮め、全体が削除された参照は `#REF!` になります。Excelと同様に、条件付き書式・入力規則・列幅・スパークラインは直前の行・列から挿入行・列へ引き継ぎ、テーブル内の挿入行には計算列の数式を入れます。テーブルの列を削除すると、その列への構造化参照は `#REF!` になります。ハイパーリンクの移動先（`location`）はExcelと同じく書き換えず、ピボットのキャッシュは更新しません。計算チェーン（`calcChain.xml`）は削除し、Excelが再生成します。テーブル内に挿入した列の既定の見出しは `Column1` 等で、ExcelのUI言語による名前（`列2` 等）とは異なることがあります。
