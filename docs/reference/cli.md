...
本文の `blocks.<block>.rows` に新しい `r<number>` 行、または新しい大文字列の列を追加すると、CLIが対応する挿入操作を探してmapping entryを再生成します。削除時は本文から対象の行・列を削除し、対応するdelete操作を残します。複数の候補操作があり一意に対応できない場合や、操作のない追加・削除は検証で拒否します。
