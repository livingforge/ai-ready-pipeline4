...
def origin_of(file: str) -> tuple[str, str]:
    ...
    Excel だけ特別扱いするのは、**1 冊の中にシートが何枚も入る**という原本側の
    事実だからである。それ以外はファイルが原本なので、束ねる単位はディレクトリに
    する ―― Java 114 本を 114 行並べると、表が「どの資料が届いたか」ではなく
    ``ls`` になる（→ 決定 19 で要件定義書に起きたのと同じ壊れ方）。
    """
