...
def test_7工程ぜんぶに設計書が出る(通し_束) -> None:
    """**束の帯も工程ごとのフォルダ分けも、1 つの段で閉じているうちは試されない。**

    `プログラム設計` を足した（3.17.0）ので 7 工程である。この束は資料が文書
    だけなので**プログラム設計書は空で出る** ―― それでよい。空で出ることが
    「この工程の語彙が正本に無い」の申告になっていて、フォルダごと出ないと
    読み手には「まだ書いていない」と「そもそも出ない」の区別が付かない。
    """
    paths, _root = 通し_束
    layers = {path.parent.name for path in paths.out.rglob("*.md")
              if path.parent != paths.out}
    assert len(layers) == 7, f"工程が {sorted(layers)} しかありません"
    索引 = (paths.out / "目次.md").read_text(encoding="utf-8")
    for 名前 in ("プロジェクト計画書", "要件定義書", "テーブル定義書",
                 "詳細設計書", "プログラム設計書", "テスト結果報告書",
                 "トレーサビリティ"):
...
def test_冊をまたぐ食い違いが課題になる(通し_束) -> None:
    ...
    """
    paths, _root = 通し_束
    課題 = (paths.out / "7_管理" / "課題管理表.md").read_text(encoding="utf-8")
...
def test_版差で写さなかったものが1枚に出る(通し_束) -> None:
    """**旧版は正本に入れない。だが落とした跡は出す。**

    この束の機能仕様書は変更履歴が未確定のまま回ってきており（実案件では
    普通の形である）、消された文言は本文と同じ資料に残っている。写さなかった
    ことを言わないと、読み手には `W046`（資料に無い）と区別が付かない ――
    **値はあるのに無いと報告されるのが、いちばん悪い形である。**

    これを `未読取` で申告していたころは、穴の 1 枚が「機械が読めなかったもの」
    として並べていた ―― 機械は読めている。決まっていないのは**どちらが現行か**
    である。
    """
    paths, _root = 通し_束
    穴 = (paths.out / "0_この設計書の穴.md").read_text(encoding="utf-8")
    assert "版差で写さなかったもの" in 穴
    assert "消された文言（端数の扱い）" in 穴
    assert "「資料に無い」ではありません" in 穴
