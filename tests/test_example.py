...
records:
  - concept: c-与信枠超過の承認
    ...
    source: { anchor: w4-h1 }
    refs: [{ rel: constrains, to: c-T_ORDER }]
# **版が併記された塊**（変更履歴が未確定で、消された文言が `w4-d1` に出ている）。
# 宣言が無いと `G034` で凍結が止まる ―― 判断しないまま両方が仕様になるのを防ぐ。
revisions:
  - { anchor: w4-h1, adopted: 変更履歴の反映後（承認期限 3 営業日）,
      dropped: 消された文言（承認期限 5 営業日）,
      reason: 本文は反映後の姿で出ているのでそちらを写した }
""",
