...
def test_csvは当てた区切りと文字コードを必ず書く(parsed) -> None:
    ...
    # **揃い方はそのまま言う。** 末尾の升が落ちた行が 1 本混ざっているので
    # 100% にはならない ―― 揃っていないことを黙ると、足りない列が
    # 「資料が空欄」に見える。
    assert "89% が 8 列" in said and "8 列でない行が 1 行あります" in said
...
def test_csvの引用符の中の区切りと改行を守る(parsed) -> None:
    ...
    **引用符・改行・カンマが同時に入った升**（`C0006` の備考）も見る ――
    1 つずつなら素朴な実装でも通ることがあるが、3 つ揃うと行の切れ目そのものを
    見失う。**末尾の升が落ちた行**（`C0007`）はそこだけ 7 列で入る（幅を
    揃えて出すのは表にするときの話で、読んだ結果を先に埋めはしない）。
    """
    table = _table(_doc(parsed, "資料/N/得意先マスタ移行.csv.md"))

    assert [len(row) for row in table] == [8, 8, 8, 8, 8, 8, 8, 7, 8]
    ...
    # 引用符・改行・カンマが同時に入っても 1 つの升のままである。
    assert '株式会社' in table[6][1] and '"運輸倉庫"' in table[6][1]
    assert "1,000,000 から 1,500,000" in table[6][7] and "月商の 3 倍" in table[6][7]
    # 末尾の升が落ちた行は、そこだけ短いまま入る（勝手に埋めない）。
    assert table[7][-1] == "営業3課"
    # 同じコードが 2 度出てきても、どちらかを落としたりはしない。
    assert [row[0] for row in table].count("C0003") == 2
...
def test_PDFはしおりで節に割れる(parsed) -> None:
    ...
    **子しおりは割り先にしない。** この 1 冊は `2.1` `2.2` `2.3` `5.1` `6.1`
    `6.2` の 6 本を第 2 階層に持っているが、節は 7 本しか出ない ―― 章より細かく
    割ると 1 本が数段落になり、出典が細かすぎて資料の姿が見えなくなる。落として
    いるのではなく割り先にしないだけなので、**本文はどれかの章に必ず入る。**

    **しおりの無い巻末の章も同じである。** 「7 未解決の課題」は目次に載って
    いない（実物の検収仕様書でごく普通に起きる）ので、直前の章に入る ――
    資料が構造を持っていないところに、機械が切れ目を作りはしない。
    """
    docs, _ = parsed
    ...
    assert made == ["01_（前書き）.md", "02_1 適用範囲.md", "03_2 確認項目.md",
                    ...
                    "06_5 是正と再検収.md", "07_6 試験実施記録.md"]
    # しおりの無い巻末の章は、直前の章の中にそのまま入っている。
    assert "7 未解決の課題" in mdio.dump(docs[f"{ACCEPT}/07_6 試験実施記録.md"])
    # 表紙は前書きに入っている（落ちていない）
...
def test_スライドは1枚がファイル1本になる(parsed) -> None:
    ...
    assert made == ["01_全体構成（A 案）.md", "02_移行対象と件数.md",
                    ...
                    "08_本提案の対象外（やらないこと）.md", "09_利用者と権限.md",
                    "10_コード体系と採番規則.md",
                    "11_新システムのゾーン構成（サブゾーン入り）.md",
                    "12_非機能要件の目標値.md"]
...
def test_非表示のスライドは読まないが言う(parsed) -> None:
    ...
    assert "13 全体構成（B 案・旧版）" in said[name]
